...
test_iter = chainer.iterators.SerialIterator(test, batchsize, repeat=False, shuffle=False) # 評価用26 ● 第 2 章　深層学習
# アップデータの登録
...
trainer.extend(extensions.PrintReport(['epoch', 'main/loss', 'validation/main/loss','main/accuracy', 'validation/main/accuracy', 'elapsed_time'] )) # 計算状の表示
#trainer.extend(extensions.dump_graph('main/loss')) # ニューラルネットワークの造
...
#trainer.extend(extensions.snapshot(), trigger=(100, 'epoch')) # 学習再開のたのスナップショット出力
